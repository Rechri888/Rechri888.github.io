var slist = {
  ...
  delete : id => { if (confirm("Vuoi rimuovere questo prodotto?")) {
  ...
  draw : () => {
    ...
    if (slist.items.length == 0) {
      slist.hlist.innerHTML = "<div class='item-row item-name'>Nessen prodotto.</div>";
    }
    ...
    // (F3) DRAW ITEMS
      ...
      for (let i in slist.items) {
        ...
        ok.type = "button";
        ok.value = slist.items[i].done ? "Non preso" : "Già preso";
